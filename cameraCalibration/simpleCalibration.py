# https://blog.csdn.net/qq_45077760/article/details/130261489

# 1. 直接使用代码获得焦距，需要提前拍摄一个矩形物体，拍摄时候相机固定，距离被拍摄物体自行设定，并一直保持此距离，背景为纯色，不要出现杂物；最后将拍摄的视频用以下代码检测：
# 2. 反复调节 ret, binary = cv2.threshold(gray, 140, 200, 60)这一行里边的三个参数，直到线条紧紧包裹住你所拍摄视频的物体，然后调整相机焦距直到左上角距离和你拍摄视频时相机到物体的距离接近为止
import cv2
